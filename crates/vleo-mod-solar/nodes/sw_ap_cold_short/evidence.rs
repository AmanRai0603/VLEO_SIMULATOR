...
#[test]
fn fixture_1() {
    let got = model::evaluate(Ratio::new(50.0), Ratio::new(10.0)).expect("the fixture case must not be refused");
    let err = relative_error(got.get(), 40.0);
    assert!(err <= 1e-12, "round numbers, checkable without a calculator — 50 - 10: got {} want 40.0, relative error {} exceeds the declared tolerance 1e-12. This is a physics disagreement, not a build failure — take it to the node owner. Do not widen the tolerance.", got.get(), err);
}

/// a sustained level of 11, four tenths above the floor — the narrowest case this row can publish, and the reason the guard at zero is load-bearing here rather than decorative
///
/// Provenance: `independent-derivation`, source `noaa_swpc`.
#[test]
fn fixture_2() {
    let got = model::evaluate(Ratio::new(11.0), Ratio::new(10.5888888889)).expect("the fixture case must not be refused");
...
fn answers_near_the_known_good_point() {
    ...
    for scale in [0.99_f64, 1.01] {
        if let Err(f) = model::evaluate(Ratio::new(17.49546836 * scale), Ratio::new(10.5888888889)) {
            refused.push(format!("sustained x{scale} -> {f}"));
        }
    }
    for scale in [0.99_f64, 1.01] {
        if let Err(f) = model::evaluate(Ratio::new(17.49546836), Ratio::new(10.5888888889 * scale)) {
            refused.push(format!("daily x{scale} -> {f}"));
        }
    }
    assert!(
        refused.is_empty(),
        "sw_ap_cold_short refuses near its own known-good point: {:?}. Either the relation is wrong in shape, or the declared domain 0 … 400 is narrower than the physics. Both are sheet questions for the node owner, not tolerances to widen.",
        refused
    );
}
...
fn every_answer_is_inside_the_declared_domain() {
    for scale in [0.001_f64, 0.1, 1.0, 10.0, 1000.0] {
        if let Ok(v) = model::evaluate(Ratio::new(17.49546836 * scale), Ratio::new(10.5888888889)) {
            assert!(v.get().is_finite(), "sw_ap_cold_short produced a value that is not a number for Ap_cold_short");
            assert!(v.get() >= 0.0 && v.get() <= 400.0, "sw_ap_cold_short answered {} for Ap_cold_short, outside its declared domain 0 … 400 — the guard did not stop it", v.get());
        }
    }
    for scale in [0.001_f64, 0.1, 1.0, 10.0, 1000.0] {
        if let Ok(v) = model::evaluate(Ratio::new(17.49546836), Ratio::new(10.5888888889 * scale)) {
            assert!(v.get().is_finite(), "sw_ap_cold_short produced a value that is not a number for Ap_cold_short");
            assert!(v.get() >= 0.0 && v.get() <= 400.0, "sw_ap_cold_short answered {} for Ap_cold_short, outside its declared domain 0 … 400 — the guard did not stop it", v.get());
        }
    }
...
fn the_same_inputs_give_the_same_answer() {
    let a = model::evaluate(Ratio::new(17.49546836), Ratio::new(10.5888888889));
    let b = model::evaluate(Ratio::new(17.49546836), Ratio::new(10.5888888889));
    match (a, b) {
        (Ok(x), Ok(y)) => {
            assert!(x.get().to_bits() == y.get().to_bits(), "sw_ap_cold_short is not deterministic for Ap_cold_short: {} then {}", x.get(), y.get());
        }
        (Err(_), Err(_)) => {}
        _ => panic!("sw_ap_cold_short refused on one call and answered on the other"),
    }
}
