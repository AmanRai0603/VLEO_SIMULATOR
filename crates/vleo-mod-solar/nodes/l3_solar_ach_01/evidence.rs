...
fn answers_near_the_known_good_point() {
    ...
    assert!(
        refused.is_empty(),
        "l3_solar_ach_01 refuses near its own known-good point: {:?}. Either the relation is wrong in shape, or the declared domain 60 … 400 is narrower than the physics. Both are sheet questions for the node owner, not tolerances to widen.",
        refused
    );
}
...
fn every_answer_is_inside_the_declared_domain() {
    for scale in [0.001_f64, 0.1, 1.0, 10.0, 1000.0] {
        if let Ok(v) = model::evaluate(Ratio::new(228.1374378829 * scale)) {
            assert!(v.get().is_finite(), "l3_solar_ach_01 produced a value that is not a number for F107_ach_long");
            assert!(v.get() >= 60.0 && v.get() <= 400.0, "l3_solar_ach_01 answered {} for F107_ach_long, outside its declared domain 60 … 400 — the guard did not stop it", v.get());
        }
    }
}
...
fn the_same_inputs_give_the_same_answer() {
    ...
    match (a, b) {
        (Ok(x), Ok(y)) => {
            assert!(x.get().to_bits() == y.get().to_bits(), "l3_solar_ach_01 is not deterministic for F107_ach_long: {} then {}", x.get(), y.get());
        }
        (Err(_), Err(_)) => {}
