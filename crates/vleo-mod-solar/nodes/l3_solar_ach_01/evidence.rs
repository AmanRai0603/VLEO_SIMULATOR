...
fn every_answer_is_inside_the_declared_domain() {
    for scale in [0.001_f64, 0.1, 1.0, 10.0, 1000.0] {
        if let Ok(v) = model::evaluate(Ratio::new(228.1374378829 * scale)) {
            assert!(v.get().is_finite(), "l3_solar_ach_01 produced a value that is not a number");
            assert!(v.get() >= 60.0 && v.get() <= 400.0, "l3_solar_ach_01 answered {}, outside its declared domain 60 … 400 — the guard did not stop it", v.get());
        }
    }
}
...
fn the_same_inputs_give_the_same_answer() {
    ...
    match (a, b) {
        (Ok(x), Ok(y)) => assert!(x.get().to_bits() == y.get().to_bits(), "l3_solar_ach_01 is not deterministic: {} then {}", x.get(), y.get()),
        (Err(_), Err(_)) => {}
