...
//! Evidence for `sw_f107_cold_short`.
//!
...
fn answers_near_the_known_good_point() {
    ...
    for scale in [0.99_f64, 1.01] {
        if let Err(f) = model::evaluate(Ratio::new(141.1088022713 * scale), Ratio::new(31.2722222222)) {
            refused.push(format!("sustained x{scale} -> {f}"));
        }
    }
    for scale in [0.99_f64, 1.01] {
        if let Err(f) = model::evaluate(Ratio::new(141.1088022713), Ratio::new(31.2722222222 * scale)) {
            refused.push(format!("daily x{scale} -> {f}"));
        }
    }
    assert!(
        refused.is_empty(),
        "sw_f107_cold_short refuses near its own known-good point: {:?}. Either the relation is wrong in shape, or the declared domain 60 … 400 is narrower than the physics. Both are sheet questions for the node owner, not tolerances to widen.",
        refused
    );
}
...
fn every_answer_is_inside_the_declared_domain() {
    for scale in [0.001_f64, 0.1, 1.0, 10.0, 1000.0] {
        if let Ok(v) = model::evaluate(Ratio::new(141.1088022713 * scale), Ratio::new(31.2722222222)) {
            assert!(v.get().is_finite(), "sw_f107_cold_short produced a value that is not a number for F107_cold_short");
            assert!(v.get() >= 60.0 && v.get() <= 400.0, "sw_f107_cold_short answered {} for F107_cold_short, outside its declared domain 60 … 400 — the guard did not stop it", v.get());
        }
    }
    for scale in [0.001_f64, 0.1, 1.0, 10.0, 1000.0] {
        if let Ok(v) = model::evaluate(Ratio::new(141.1088022713), Ratio::new(31.2722222222 * scale)) {
            assert!(v.get().is_finite(), "sw_f107_cold_short produced a value that is not a number for F107_cold_short");
            assert!(v.get() >= 60.0 && v.get() <= 400.0, "sw_f107_cold_short answered {} for F107_cold_short, outside its declared domain 60 … 400 — the guard did not stop it", v.get());
        }
    }
...
fn the_same_inputs_give_the_same_answer() {
    let a = model::evaluate(Ratio::new(141.1088022713), Ratio::new(31.2722222222));
    let b = model::evaluate(Ratio::new(141.1088022713), Ratio::new(31.2722222222));
    match (a, b) {
        (Ok(x), Ok(y)) => {
            assert!(x.get().to_bits() == y.get().to_bits(), "sw_f107_cold_short is not deterministic for F107_cold_short: {} then {}", x.get(), y.get());
        }
        (Err(_), Err(_)) => {}
        _ => panic!("sw_f107_cold_short refused on one call and answered on the other"),
    }
}
