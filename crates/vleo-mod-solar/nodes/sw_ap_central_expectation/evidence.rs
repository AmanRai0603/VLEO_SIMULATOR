...
//! Evidence for `sw_ap_central_expectation`.
//!
...
///
/// Migrated from `prf_density.m:215 (01_kernel/sw_study/06_density, local function designWindow_, B.fc27.pred(end))`. A second opinion and never an expected
...
fn parity_grid() {
    ...
    assert!(
        err <= TOL,
        "sw_ap_central_expectation: this row says {got} and the prior implementation `prf_density.m:215 (01_kernel/sw_study/06_density, local function designWindow_, B.fc27.pred(end))` says {expected} — {err} apart, beyond {TOL}.\nThis is a finding about one of the two implementations, not a build failure and not proof this one is wrong. Take it to the node owner. Do not widen parity_tolerance and do not edit parity.csv to agree."
    );
}
