...
fn every_answer_is_inside_the_declared_domain() {
    for scale in [0.001_f64, 0.1, 1.0, 10.0, 1000.0] {
        if let Ok(v) = model::evaluate(Ratio::new(80.0 * scale)) {
            assert!(v.get().is_finite(), "sw_exceedance_phase produced a value that is not a number");
            assert!(v.get() >= 0.0 && v.get() <= 1.0, "sw_exceedance_phase answered {}, outside its declared domain 0 … 1 — the guard did not stop it", v.get());
        }
    }
}
...
fn the_same_inputs_give_the_same_answer() {
    ...
    match (a, b) {
        (Ok(x), Ok(y)) => assert!(x.get().to_bits() == y.get().to_bits(), "sw_exceedance_phase is not deterministic: {} then {}", x.get(), y.get()),
        (Err(_), Err(_)) => {}
