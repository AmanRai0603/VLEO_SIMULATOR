...
fn every_answer_is_inside_the_declared_domain() {
    for scale in [0.001_f64, 0.1, 1.0, 10.0, 1000.0] {
        if let Ok(v) = model::evaluate(Ratio::new(80.0 * scale)) {
            assert!(v.get().is_finite(), "sw_design_safe_duration produced a value that is not a number");
            assert!(v.get() >= 9467280.0 && v.get() <= 94672800.0, "sw_design_safe_duration answered {}, outside its declared domain 9467280 … 94672800 — the guard did not stop it", v.get());
        }
    }
}
...
fn the_same_inputs_give_the_same_answer() {
    ...
    match (a, b) {
        (Ok(x), Ok(y)) => assert!(x.get().to_bits() == y.get().to_bits(), "sw_design_safe_duration is not deterministic: {} then {}", x.get(), y.get()),
        (Err(_), Err(_)) => {}
