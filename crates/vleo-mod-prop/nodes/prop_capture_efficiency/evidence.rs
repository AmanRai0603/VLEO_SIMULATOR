...
//! Evidence for `prop_capture_efficiency`.
//!
...
fn fixture_0() {
    ...
    let err = relative_error(got.get(), 0.40909);
    assert!(err <= 0.0001, "IRS-class baseline intake: got {} want 0.40909, relative error {} exceeds the declared tolerance 0.0001. This is a physics disagreement, not a build failure — take it to the node owner. Do not widen the tolerance.", got.get(), err);
}
...
///
/// Derived from `IRS-class baseline intake` and the declared domain 0 … 1.
///
...
fn answers_near_the_known_good_point() {
    ...
    assert!(
        refused.is_empty(),
        "prop_capture_efficiency refuses near its own known-good point: {:?}. Either the relation is wrong in shape, or the declared domain 0 … 1 is narrower than the physics. Both are sheet questions for the node owner, not tolerances to widen.",
        refused
    );
}
