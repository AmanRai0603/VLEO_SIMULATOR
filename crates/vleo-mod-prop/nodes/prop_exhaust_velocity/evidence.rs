...
///
/// Derived from `300 V, atomic oxygen mixture` and the declared domain 1000 … 500000.
///
...
fn answers_near_the_known_good_point() {
    ...
    for scale in [0.99_f64, 1.01] {
        if let Err(f) = model::evaluate(Voltage::new(300.0 * scale), MolarMass::new(0.01872)) {
            refused.push(format!("vb x{scale} -> {f}"));
        }
    }
    for scale in [0.99_f64, 1.01] {
        if let Err(f) = model::evaluate(Voltage::new(300.0), MolarMass::new(0.01872 * scale)) {
            refused.push(format!("m x{scale} -> {f}"));
        }
    }
    assert!(
        refused.is_empty(),
        "prop_exhaust_velocity refuses near its own known-good point: {:?}. Either the relation is wrong in shape, or the declared domain 1000 … 500000 is narrower than the physics. Both are sheet questions for the node owner, not tolerances to widen.",
        refused
    );
}
