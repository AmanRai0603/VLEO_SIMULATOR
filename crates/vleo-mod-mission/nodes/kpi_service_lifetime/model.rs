...
///
/// Deliberately measured against the uncontrolled lifetime rather than the
/// propelled one, so the answer does not depend on the propulsion system
/// working.
...
pub fn evaluate(req: Time, ach: Time) -> Result<Ratio, Fault> {
