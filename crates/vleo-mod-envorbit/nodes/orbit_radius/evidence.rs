...
fn every_answer_is_inside_the_declared_domain() {
    for scale in [0.001_f64, 0.1, 1.0, 10.0, 1000.0] {
        if let Ok(v) = model::evaluate(Length::new(250000.0 * scale)) {
            assert!(v.get().is_finite(), "orbit_radius produced a value that is not a number for r");
            assert!(v.get() >= 6400000.0 && v.get() <= 7000000.0, "orbit_radius answered {} for r, outside its declared domain 6400000 … 7000000 — the guard did not stop it", v.get());
        }
    }
}
...
fn the_same_inputs_give_the_same_answer() {
    ...
    match (a, b) {
        (Ok(x), Ok(y)) => {
            assert!(x.get().to_bits() == y.get().to_bits(), "orbit_radius is not deterministic for r: {} then {}", x.get(), y.get());
        }
        (Err(_), Err(_)) => {}
