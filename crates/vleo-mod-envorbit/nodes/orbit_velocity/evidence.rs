...
///
/// Derived from `250 km circular` and the declared domain 7000 … 8200.
///
...
fn answers_near_the_known_good_point() {
    ...
    assert!(
        refused.is_empty(),
        "orbit_velocity refuses near its own known-good point: {:?}. Either the relation is wrong in shape, or the declared domain 7000 … 8200 is narrower than the physics. Both are sheet questions for the node owner, not tolerances to widen.",
        refused
    );
}
