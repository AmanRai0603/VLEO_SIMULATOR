...
///
/// Derived from `250 km circular` and the declared domain 1.4835298641951802 … 2.007128639793479.
///
...
fn answers_near_the_known_good_point() {
    ...
    for scale in [0.99_f64, 1.01] {
        if let Err(f) = model::evaluate(Length::new(6628137.0 * scale), Ratio::new(0.0)) {
            refused.push(format!("r x{scale} -> {f}"));
        }
    }
    for scale in [0.99_f64, 1.01] {
        if let Err(f) = model::evaluate(Length::new(6628137.0), Ratio::new(0.0 * scale)) {
            refused.push(format!("e x{scale} -> {f}"));
        }
    }
    assert!(
        refused.is_empty(),
        "orbit_sun_sync_inclination refuses near its own known-good point: {:?}. Either the relation is wrong in shape, or the declared domain 1.4835298641951802 … 2.007128639793479 is narrower than the physics. Both are sheet questions for the node owner, not tolerances to widen.",
        refused
    );
}
