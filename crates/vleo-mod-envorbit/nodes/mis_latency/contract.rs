...
pub const OUTPUT_VARS: &[&str] = &["mis_latency"];
...
pub fn call(inputs: &[f64], outputs: &mut [f64]) -> Result<(), Fault> {
    ...
    let tw: Time = Time::new(inputs[0]);
    let td: Time = Time::new(inputs[1]);
    let tp: Time = Time::new(inputs[2]);
    let tdl: Time = Time::new(inputs[3]);
    let answer = super::model::evaluate(tw, td, tp, tdl)?;
