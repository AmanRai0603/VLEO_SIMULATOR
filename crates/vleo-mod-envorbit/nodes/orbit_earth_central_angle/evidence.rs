...
fn every_answer_is_inside_the_declared_domain() {
    for scale in [0.001_f64, 0.1, 1.0, 10.0, 1000.0] {
        if let Ok(v) = model::evaluate(Length::new(6628137.0 * scale), Angle::new(0.17453293)) {
            assert!(v.get().is_finite(), "orbit_earth_central_angle produced a value that is not a number for lambda");
            assert!(v.get() >= 0.0 && v.get() <= 1.5009831567151235, "orbit_earth_central_angle answered {} for lambda, outside its declared domain 0 … 1.5009831567151235 — the guard did not stop it", v.get());
        }
    }
    for scale in [0.001_f64, 0.1, 1.0, 10.0, 1000.0] {
...
fn the_same_inputs_give_the_same_answer() {
    ...
    match (a, b) {
        (Ok(x), Ok(y)) => {
            assert!(x.get().to_bits() == y.get().to_bits(), "orbit_earth_central_angle is not deterministic for lambda: {} then {}", x.get(), y.get());
        }
        (Err(_), Err(_)) => {}
