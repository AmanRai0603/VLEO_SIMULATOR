...
///
/// Derived from `250 km, baseline vehicle` and the declared domain 0 … 10.
///
...
fn answers_near_the_known_good_point() {
    ...
    for scale in [0.99_f64, 1.01] {
        if let Err(f) = model::evaluate(Pressure::new(0.0019938 * scale), Ratio::new(2.85807), Area::new(0.3327)) {
            refused.push(format!("q x{scale} -> {f}"));
        }
    }
    for scale in [0.99_f64, 1.01] {
        if let Err(f) = model::evaluate(Pressure::new(0.0019938), Ratio::new(2.85807 * scale), Area::new(0.3327)) {
            refused.push(format!("cd x{scale} -> {f}"));
        }
    }
    for scale in [0.99_f64, 1.01] {
        if let Err(f) = model::evaluate(Pressure::new(0.0019938), Ratio::new(2.85807), Area::new(0.3327 * scale)) {
            refused.push(format!("a x{scale} -> {f}"));
        }
    }
    assert!(
        refused.is_empty(),
        "aero_drag_force refuses near its own known-good point: {:?}. Either the relation is wrong in shape, or the declared domain 0 … 10 is narrower than the physics. Both are sheet questions for the node owner, not tolerances to widen.",
        refused
    );
}
