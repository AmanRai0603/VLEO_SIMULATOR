...
pub const OUTPUT_VARS: &[&str] = &["aero_drag_acceleration"];
/// The SI unit this node's own answer crosses the boundary in.
pub const OUTPUT_UNIT: Unit = Acceleration::UNIT;
/// The SI unit of each published variable, in `OUTPUT_VARS` order.
pub const OUTPUT_UNITS: &[Unit] = &[Acceleration::UNIT];
...
pub fn call(inputs: &[f64], outputs: &mut [f64]) -> Result<(), Fault> {
    ...
    let d: Force = Force::new(inputs[0]);
    let m: Mass = Mass::new(inputs[1]);
    let answer = super::model::evaluate(d, m)?;
