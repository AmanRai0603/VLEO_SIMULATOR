...
///
/// `Ap_day_sys = l3_solar_interface.ap_hotday`
///
/// Source: `noaa_swpc`
///
...
pub fn evaluate(crossing: Ratio) -> Result<Ratio, Fault> {
    ...
    if !answer.is_finite() {
        return Err(Fault::Degenerate { node: NODE_ID, field: "Ap_day_sys", reason: "the computation produced a value that is not a number" });
    }
    if answer.get() < 0.0 {
        return Err(Fault::OutOfDomain { node: NODE_ID, field: "Ap_day_sys", value: answer.get(), bound: 0.0, edge: Edge::Lower, unit: Ratio::UNIT, reason: "the crossing's own floor, restated. A row that narrowed the range it received would be changing the answer while appearing to relay it" });
    }
    if answer.get() > 400.0 {
        return Err(Fault::OutOfDomain { node: NODE_ID, field: "Ap_day_sys", value: answer.get(), bound: 400.0, edge: Edge::Upper, unit: Ratio::UNIT, reason: "the crossing's own ceiling, restated. This is a single-day value, so it is the one most likely of the pair to approach it" });
    }
    Ok(answer)
}
